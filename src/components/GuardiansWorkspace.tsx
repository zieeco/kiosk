...
export default function GuardiansWorkspace() {
  ...
  const filteredGuardians = guardians.filter((guardian: any) => {
    ...
    const matchesResident = !filterResident || 
      guardian.residentIds?.includes(filterResident);
              ...
              <div className="border border-gray-300 rounded-md p-3 max-h-48 overflow-y-auto">
                {residents.map((resident: any) => (
                  <label key={resident.id} className="flex items-center mb-2">
                    <input
                      type="checkbox"
                      checked={editingGuardian.residentIds?.includes(resident.id)}
                      onChange={(e) => {
                        const newResidentIds = e.target.checked
                          ? [...(editingGuardian.residentIds || []), resident.id]
                          : editingGuardian.residentIds.filter((id: string) => id !== resident.id);
                        setEditingGuardian({ ...editingGuardian, residentIds: newResidentIds });
                      }}
                      className="mr-2"
                    />
                    <span className="text-sm">{resident.name} ({resident.location})</span>
                  </label>
                ))}
              </div>
            </div>
                ...
                filteredGuardians.map((guardian: any) => (
                  <tr key={guardian.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <div className="text-sm text-gray-900">
                        {guardian.residentNames && guardian.residentNames.length > 0 ? (
                          <div className="flex flex-wrap gap-1">
                            {guardian.residentNames.map((name: string, idx: number) => (
                              <span
                                key={idx}
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">
                        {guardian.createdAt ? new Date(guardian.createdAt).toLocaleDateString() : "Unknown"}
                      </div>
                    </td>
